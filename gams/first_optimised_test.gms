...
* accepted  RL\ in \ out:
                + sum(s_out_RL, sum(s_in_RL, sum(t_block$map_quarter_block(t_quarter, t_block), (omega_RL_in(t_block, s_in_RL) * omega_RL_out(t_block, s_out_RL)))      * (
                        + (0.0625 * sum(t_block$map_quarter_block(t_quarter, t_block), (Q_in_RL(t_block, s_in_RL)        * p_in_RL(t_block, s_in_RL)))                        )
                        + (0.0625 * sum(t_block$map_quarter_block(t_quarter, t_block), (Q_out_RL(t_block, s_out_RL)      * p_out_RL(t_block, s_out_RL))))
                        + (0.25 * sum(t_hour$map_quarter_hour(t_quarter, t_hour),           (Q_rB_DA(t_hour, s_in_RL, s_out_RL)              * DA_expV(t_hour)  )))
                        + (RA_in_expV * sum(s_in_RA, Q_inrB_RA(t_quarter, s_in_RA, s_in_RL, s_out_RL) * omega_RA_inrB_call(t_quarter, s_in_RA)))        
                        + (RA_out_expV * sum(s_out_RA, Q_outrB_RA(t_quarter, s_out_RA, s_in_RL, s_out_RL)  * omega_RA_outrB_call(t_quarter, s_out_RA)    )  ) ) ) )
*accepted RL in     \ declined out:
                + sum(s_out_RL, sum(s_in_RL, sum(t_block$map_quarter_block(t_quarter, t_block), (omega_RL_in(t_block, s_in_RL) * (1-omega_RL_out(t_block, s_out_RL))))   * (
                        + (0.0625 * sum(t_block$map_quarter_block(t_quarter, t_block), (Q_in_RL(t_block, s_in_RL)        * p_in_RL(t_block, s_in_RL)))                        )
                        + (0.25 * sum(t_hour$map_quarter_hour(t_quarter, t_hour),           (Q_rI_DA(t_hour, s_in_RL, s_out_RL)              * DA_expV(t_hour)    )))
                        + (RA_in_expV * sum(s_in_RA, Q_inrI_RA(t_quarter, s_in_RA, s_in_RL, s_out_RL) * omega_RA_inrI_call(t_quarter, s_in_RA)))      
                        + (RA_out_expV * sum(s_out_RA, Q_outrI_RA(t_quarter, s_out_RA, s_in_RL, s_out_RL)  * omega_RA_outrI_call(t_quarter, s_out_RA)    )  ) ) ))                           
*declined RL in     \ accepted out:
                + sum(s_out_RL, sum(s_in_RL, sum(t_block$map_quarter_block(t_quarter, t_block), ((1-omega_RL_in(t_block, s_in_RL)) * omega_RL_out(t_block, s_out_RL)))   * (
                        + (0.0625 * sum(t_block$map_quarter_block(t_quarter, t_block), (Q_out_RL(t_block, s_out_RL)      * p_out_RL(t_block, s_out_RL))))
                        + (0.25 * sum(t_hour$map_quarter_hour(t_quarter, t_hour),           (Q_rO_DA(t_hour, s_in_RL, s_out_RL)              * DA_expV(t_hour)    )))
                        + (RA_in_expV * sum(s_in_RA, Q_inrO_RA(t_quarter, s_in_RA, s_in_RL, s_out_RL) * omega_RA_inrO_call(t_quarter, s_in_RA)))       
                        + (RA_out_expV * sum(s_out_RA, Q_outrO_RA(t_quarter, s_out_RA, s_in_RL, s_out_RL) * omega_RA_outrO_call(t_quarter, s_out_RA)    )  ) ) ))
*declined RL in\ out:
                + sum(s_out_RL, sum(s_in_RL, sum(t_block$map_quarter_block(t_quarter, t_block), (1-(omega_RL_in(t_block, s_in_RL) * (1-omega_RL_out(t_block, s_out_RL)))))  * (
                        + (0.25 * sum(t_hour$map_quarter_hour(t_quarter, t_hour),           (Q_rN_DA(t_hour, s_in_RL, s_out_RL)              * DA_expV(t_hour)    )))
                        + (RA_in_expV * sum(s_in_RA, Q_inrN_RA(t_quarter, s_in_RA, s_in_RL, s_out_RL) * omega_RA_inrN_call(t_quarter, s_in_RA)) )       
                        + (RA_out_expV * sum(s_out_RA, Q_outrN_RA(t_quarter, s_out_RA, s_in_RL, s_out_RL) * omega_RA_outrN_call(t_quarter, s_out_RA))  ) ) ) )
                );
