...
* accepted  RL\ in \ out:
                sum(s_out_RL,sum(s_in_RL, (omega_RL_in(s_in_RL)*omega_RL_out(s_out_RL))   *(
                        (Q_in_RL (s_in_RL) *p_in_RL(s_in_RL)  + Q_out_RL(s_out_RL) * p_out_RL(s_out_RL))
                        + sum(s_DA, omega_DA(s_DA)   *(
                            Q_rRL_DA(s_DA) * p_DA(s_DA) 
                            + sum(s_in_RA, Q_inrRLDA_RA(s_in_RA) * p_in_RA(s_in_RA) * omega_RA_in(s_in_RA))
                            + sum(s_out_RA, Q_outrRLDA_RA(s_out_RA) * p_out_RA(s_out_RA) * omega_RA_out(s_out_RA))))
                        + sum(s_DA, (1-omega_DA(s_DA))  *(
                            sum(s_in_RA, Q_inrRL_RA(s_in_RA) * p_in_RA(s_in_RA) * omega_RA_in(s_in_RA))
                            + sum(s_out_RA, Q_outrRL_RA(s_out_RA) * p_out_RA(s_out_RA) * omega_RA_out(s_out_RA) ) )))))
*accepted RL in     \ declined out:
                +sum(s_out_RL,sum(s_in_RL,(omega_RL_in(s_in_RL)*(1-omega_RL_out(s_out_RL))   *(
                        (Q_in_RL(s_in_RL) * p_in_RL(s_in_RL))
                        + sum(s_DA, omega_DA(s_DA) *(
                            Q_rRL_DA(s_DA) * p_DA(s_DA) 
                            + sum(s_in_RA, Q_inrRLDA_RA(s_in_RA) * p_in_RA(s_in_RA) * omega_RA_in(s_in_RA))
...
*declined RL in\ accepted out:
                +sum(s_out_RL,sum(s_in_RL,(1-omega_RL_in(s_in_RL))*omega_RL_out(s_out_RL))   *(
                        (Q_out_RL(s_out_RL) * p_out_RL(s_out_RL))
                        + sum(s_DA, omega_DA(s_DA)  *(
                                (Q_rRL_DA(s_DA) * p_DA(s_DA))
...
*c_in_RL =l= p(s_in_RL) + M * B_in_RL(s_in_RL)\quad\forall s_in_RL;
*c_in_RL =g= p(s_in_RL) - M * (1 - B_in_RL(s_in_RL))\quad\forall s_in_RL;
*c_out_RL =l= p(s_out_RL) + M * B_out_RL(s_out_RL)\quad\forall s_out_RL;
*c_out_RL =g= p(s_out_RL) - M * (1 - B_out_RL(s_out_RL))\quad\forall s_out_RL;
